...
| Kimi | ✅ 已校准 | `.segment-user` / `.segment-assistant` 轮次。原生工具栏注入（克隆 `simple-button` / `icon-button`，插在站点复制按钮旁）。`div.paragraph` / `segment-code` / `markdown-table` 走转换器拦截；公式从 KaTeX 渲染树反编译回 LaTeX，粘贴为 OneNote 原生公式。 |
